...
import time
import base64
import PyPDF2  # Thêm thư viện này để đếm trang nhanh
...
[data-testid="stFileUploader"] { border: 2px dashed #93c5fd; padding: 25px; border-radius: 18px; background: white; transition: 0.3s; }
[data-testid="stFileUploader"]:hover { border-color:#3b82f6; }
div.stButton > button { background: linear-gradient(135deg,#3b82f6,#22c55e); color:white; border:none; border-radius:12px; padding:12px 24px; font-weight:600; font-size:15px; box-shadow:0 4px 14px rgba(0,0,0,0.15); transition: all 0.25s ease; width: 100%; }
div.stButton > button:hover { transform: translateY(-2px) scale(1.01); }
...
.global-fill { height:100%; border-radius:999px; transition: width 0.4s ease; position:relative; }
.global-text { position:absolute; width:100%; text-align:center; font-size:12px; font-weight:700; top:0; line-height:20px; color: #1e293b; }
...
def ocr_extract(img):
    def read(image):
        # PSM 6 phù hợp cho các khối văn bản đồng nhất
    ...
    w, h = img.size
    # Kiểm tra các biến thể quan trọng nhất để tiết kiệm thời gian
    # 1. Ảnh gốc | 2. Cắt phần trên (thường chứa thông tin) | 3. Xoay ngược (nếu scan ngược)
    variants = [
        img,
...
<div class="global-wrap">
    <div class="global-meta">
        <div>⚡ Tiến độ tổng: {percent}%</div>
        <div>⏳ Còn lại khoảng: {eta_text}</div>
    </div>
...
def extract_pdf(file, box, global_box, start_time, processed_pages, total_pages_all):
    results = []
    # Giảm DPI xuống 120 (đủ nét để OCR nhưng nhanh hơn 150 nhiều)
...
if uploaded_files:
    ...
    if st.session_state.processing:
        ...
        # ĐẾM TRANG NHANH BẰNG PYPDF2 (Không tốn CPU/RAM như convert_from_bytes)
        total_pages_all = 0
        for f in uploaded_files:
            try:
                pdf_reader = PyPDF2.PdfReader(f)
                total_pages_all += len(pdf_reader.pages)
                f.seek(0)
            except:
                total_pages_all += 1 # Backup nếu file lỗi
