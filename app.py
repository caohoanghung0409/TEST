...
# EXTRACT
# =========================
def extract_pdf(file, box, global_box, processed, total):
    results = []
    images = convert_from_bytes(file.read(), dpi=150)

    for i, img in enumerate(images, start=1):
        processed[0] += 1
        percent = int((processed[0] / total) * 100)

        global_box.markdown(render_global_bar(percent), unsafe_allow_html=True)

        box.markdown(f"""
        <div class="file-row">
        📄 {file.name} - Trang {i}/{len(images)}
        </div>
        """, unsafe_allow_html=True)
    ...
    if st.session_state.processing:
        ...
        with pd.ExcelWriter(excel_file.name, engine='openpyxl') as writer:
            ...
            for i, f in enumerate(uploaded_files):
                ...
                data = extract_pdf(f, boxes[i], global_box, processed, total_pages)
